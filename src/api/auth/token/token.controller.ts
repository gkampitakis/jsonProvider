...
import { _Logger, Logger } from '../../../util/decorators/logger'; //TODO: change to pino logger
...
class TokenController extends ControllerModule {
	...
	public authenticate(req: Request, res: Response) {
		passport.authenticate(
			'local',
			async (err: Error, user: UserI, info: string) => {
				...
				try {
					const { token, _id: userId } = (await this.tokenService.create(
						user._id,
						'authorization'
					)) as TokenI;
	...
	public async invalidateToken(req: Request, res: Response) {
		try {
			...
			if (!token)
				return this.handleError(res, new Error('Token not provided'), 400);
